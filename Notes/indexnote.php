...
<body>
    <div class="container">
        <h1 class="mt-2 mb-3 text-center text-primary">Note des Étudiants</h1>
            ...
            <div class="col-auto">
                <select name="subject_id" class="form-select" onchange="fetchStudents(this.value)" required>
                    ...
                    <?php foreach ($subjects as $subject) { ?>
                        <option value="<?php echo htmlspecialchars($subject['id']); ?>"><?php echo htmlspecialchars($subject['nom']); ?></option>
                    <?php } ?>
                </select>
            </div>
        </form>
    ...
    <script>
        function fetchStudents(moduleId) {
            ...
            xhr.open("GET", "note.php?module_id=" + encodeURIComponent(moduleId), true);
